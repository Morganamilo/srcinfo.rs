...
macro_rules! merge {
    ($slf:ident, $base:ident, $field:ident) => {
        ...
        {
            $base.$field = $slf.srcinfo.package.$field.clone();
        }
    };
}
...
macro_rules! merge_arch_string {
    ($slf:ident, $base:ident, $field:ident) => {
        for arch_string in &$slf.srcinfo.package.$field {
            if !has_override(
                &$slf.empty_overrides,
                stringify!($field),
...
fn append_arch_strings(arch_strings: &mut Vec<ArchVec>, arch: Option<&str>, value: &str) {
        ...
        .find(|a| arch == a.arch.as_ref().map(|x| x.as_str()))
    {
        ...
        arch_strings.push(ArchVec::from_vec(arch, vec![value.to_string()]));
    }
}
...
impl Parser {
    ...
    fn merge_current_package(&mut self) {
        if let Some(package) = self.srcinfo.packages.last_mut() {
    ...
    fn check_missing(&self) -> Result<(), ErrorKind> {
        Err(ErrorKind::MissingField(
            if self.srcinfo.base.pkgbase.is_empty() {
                "pkgbase"
            } else if self.srcinfo.packages.is_empty() {
                "pkgname"
    ...
    fn push_pkg(&mut self, pkgname: &str) {
        ...
        self.has_pkg = true;
        self.srcinfo.packages.push(pkg);
    }
    ...
    fn set_field(&mut self, key_arch: &str, value: Option<&str>) -> Result<(), ErrorKind> {
        ...
        if let Some(arch) = arch {
            let base = &self.srcinfo.package;
            let pkg = self.srcinfo.packages.last();
            let pkg = pkg.unwrap_or(&self.srcinfo.package);
    ...
    fn match_pkg(&mut self, key: &str, value: &str) -> bool {
        let pkg = self.srcinfo.packages.last_mut();
        let pkg = pkg.unwrap_or(&mut self.srcinfo.package);
...
mod tests {
    ...
    fn test_append_arch_strings() {
        let mut arch_strings = vec![ArchVec::from_arch("x86_64")];
        append_arch_strings(&mut arch_strings, Some("arm"), "foo");

        assert_eq!(
            arch_strings,
            vec![
                ArchVec::from_arch("x86_64"),
                ArchVec::from_vec(Some("arm"), vec!["foo".to_string()]),
            ]
        );
        ...
        assert_eq!(
            arch_strings,
            vec![ArchVec::from_vec(Some("x86_64"), vec!["foo".to_string()]),]
        );
        ...
        assert_eq!(
            arch_strings,
            vec![
                ArchVec::from_vec(Some("x86_64"), vec!["foo".to_string(), "a".to_string()]),
                ArchVec::from_vec(Some("arm"), vec!["bar".to_string(), "b".to_string()]),
            ]
        );
    }
}
